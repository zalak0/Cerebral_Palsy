                          ...
                          int *out_idx, int out_cap)
{
    ...
    for(int i=1;i<n-1;i++){
        if(!(rms[i] > rms[i-1] && rms[i] > rms[i+1])) continue;
        if(rms[i] < height_thr) continue;
        if(i - last_pk < distance) continue;

        // prominence approx (similar to scipy-ish)
        float left_min = rms[i];
        for(int j=i-1;j>=0;j--){
            if(rms[j] > rms[i]) break;
            if(rms[j] < left_min) left_min = rms[j];
        }
        float right_min = rms[i];
        for(int j=i+1;j<n;j++){
            if(rms[j] > rms[i]) break;
            if(rms[j] < right_min) right_min = rms[j];
        }
        float base = fmaxf(left_min, right_min);
        float prom = rms[i] - base;
        if(prom < prom_thr) continue;
                                ...
                                CL_Debug *dbg)
{
    ...
    for(int i=0;i<n;i++){
        ...
        if(v){
            voiced_cnt++;
            if(flat[i] < flat_thr && zcr[i] < zcr_thr) tonal_cnt++;
        }
    }
    ...
    int zn=0;
    for(int i=0;i<n;i++) if(st->voiced_tmp[i]) tmpz[zn++] = zcr[i];
                           ...
                           float dyn)
{
    ...
    int l0 = pk - w; if(l0 < 0) l0 = 0;
    int r0 = pk + w; if(r0 > n-1) r0 = n-1;

    if(pk - l0 < 5 || r0 - pk < 5) return false;

    int li=l0;
    float left_min=rms[l0];
    for(int i=l0;i<pk;i++){
        if(rms[i] < left_min){ left_min=rms[i]; li=i; }
    }

    int ri=pk;
    float right_min=rms[pk];
    for(int i=pk;i<=r0;i++){
        if(rms[i] < right_min){ right_min=rms[i]; ri=i; }
    }
    ...
    if(rise < rise_thr*dyn || fall < fall_thr*dyn) return false;

    float floor_limit = noise_floor + floor_tol*dyn;
    if(left_min > floor_limit || right_min > floor_limit) return false;
                            ...
                            CL_Debug *dbg)
{
    ...
    if(peak_n < min_peaks || peak_n > max_peaks) return false;
    ...
    for(int i=0;i<peak_n-1;i++){
        ...
        if(dt >= min_syllable_gap && dt <= max_syllable_gap) intervals[int_n++] = dt;
    }
    if(int_n < 2) return false;

    double sum=0,sum2=0;
    for(int i=0;i<int_n;i++){ sum += intervals[i]; sum2 += intervals[i]*intervals[i]; }
    float mean = (float)(sum/(double)int_n);
    float var  = (float)(sum2/(double)int_n - mean*mean);
    if(var<0) var=0;
    float std  = sqrtf(var);
    float cv   = std/(mean+EPS);
    if(cv > regularity_cv_thr) return false;

    // smoothness: mean abs(second diff) on normalized rms
    float rmax=0.0f;
    for(int i=0;i<n_frames;i++) if(rms[i] > rmax) rmax=rms[i];
    ...
    if(smoothness > envelope_smoothness_thr) return false;
    ...
    for(int i=0;i<pairs;i++){
        ...
        bool ok=false;
        for(int k=a;k<b;k++){
            if(rms[k] <= floor_thr){ ok=true; break; }
        }
        if(ok) floor_returns++;
    }
    float floor_return_ratio = (pairs>0)?((float)floor_returns/(float)pairs):0.0f;
    if(floor_return_ratio < floor_return_ratio_thr) return false;
    ...
    if(harmonicity < harmonicity_thr) return false;

    float cent_med = median_copy(cent, n_frames, st->rms_tmp, CL_MAX_FRAMES);
    if(!(cent_med >= speech_centroid_lo && cent_med <= speech_centroid_hi)) return false;
                          ...
                          CL_Debug *dbg)
{
    ...
    if(peak_n >= 3){
        ...
        if(short_n >= 2){
            ...
            int k=0;
            for(int i=0;i<int_n;i++) if(short_mask[i]) tmp[k++] = intervals[i];
            double sum=0,sum2=0;
            for(int i=0;i<k;i++){ sum += tmp[i]; sum2 += tmp[i]*tmp[i]; }
        ...
        // max fast run
        int cur=1; max_fast_run=1;
        for(int i=0;i<int_n;i++){
            if(short_mask[i]){
                cur++;
                if(cur>max_fast_run) max_fast_run=cur;
            }else cur=1;
        }
        ...
        while(i<int_n){
            if(short_mask[i]){
                int s=i;
                while(i<int_n && short_mask[i]) i++;
                int run_len = i - s;
                int cluster_peaks = run_len + 1;
                if(cluster_peaks >= MIN_CLUSTER_PEAKS) cluster_count++;
            }else i++;
        }
    }
...
const char* cl_label_str(CL_Label lab){
    switch(lab){
        case CL_HUM:   return "hum";
        case CL_LAUGH: return "laugh";
        case CL_CRY:   return "cry";
...
bool cl_feed_mono_block(CL_State *st, const int16_t *mono, int n){
    ...
    for(int i=0;i<n;i++){
        ...
        if(st->fb_fill >= st->n_fft && st->hop_acc >= st->hop){
            ...
            if(st->hist_len < CL_MAX_FRAMES) st->hist_len++;
...
CL_Label cl_classify_latest(CL_State *st, CL_Debug *dbg){
    if(!st) return CL_UNKNOWN;
    if(dbg) memset(dbg, 0, sizeof(*dbg));
    if(st->hist_len < 10) return CL_UNKNOWN;
    ...
    float peak_max=0.0f;
    for(int i=0;i<n;i++) if(st->rms_tmp[i] > peak_max) peak_max = st->rms_tmp[i];
    ...
    bool cry=false;
    if(peak_n <= 3) cry = (cry_good>=1) && (burst_ratio>=0.34f);
    else            cry = (cry_good>=2) && (burst_ratio>=0.60f);
